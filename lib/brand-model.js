...
export function computeEstimate({
  ...
  minRate, medianRate, maxRate, markupPct, floorWithFonts, floorNoFonts
}) {
  const A    = coeffA   ?? COEFF_A;
  const rMin = minRate  ?? MIN_RATE;
  const rMed = medianRate ?? MEDIAN_RATE;
  const rMax = maxRate  ?? MAX_RATE;
  const mult = markupPct != null ? 1 + markupPct / 100 : MARKUP_MULT;
  const flW  = floorWithFonts ?? FLOOR_WITH_FONTS;
  const flN  = floorNoFonts   ?? FLOOR_NO_FONTS;
  const EXP_B = EXP_B_MAP[clientType];
  if (EXP_B === undefined) throw new Error(`Unknown clientType: "${clientType}"`);
  ...
  const eafValue = ['vision','timeline','revisions','stakeholders']
    .reduce((prod, k) => {
      const idx = eaf?.[k] ?? 1;
      const table = EAF_MAP[k];
      if (idx < 0 || idx >= table.length) throw new RangeError(`Invalid EAF index: ${k}=${eaf?.[k]}`);
      return prod * table[idx];
    }, 1);
  ...
  const cost_low  = (E - SD) * rMin;
  const cost_mid  = E * rMed;
  const cost_high = (E + SD) * rMax;
  const fixed_raw = cost_high * mult;
  const fixed_disc = isPackage ? fixed_raw * (1 - PACKAGE_DISCOUNT) : fixed_raw;
  const floor = licensedFonts ? flW : flN;
  ...
  return {
    ...
    EXP_B, eafValue, H_base, H_adj, E, SD,
    cost_low, cost_mid, cost_high, fixed_raw, fixed_disc, fixed_quote,
    floor, floor_triggered, isPackage, licensedFonts, breakdown,
    minRate: rMin, medianRate: rMed, maxRate: rMax, markupPct: (mult - 1) * 100
  };
}
